...
import dev.phonchai.datetime.picker.component.time.Header;
import dev.phonchai.datetime.picker.component.time.PanelClock;
import dev.phonchai.datetime.picker.component.time.TimeSelectionModel;
import dev.phonchai.datetime.picker.component.time.event.TimeActionListener;
import dev.phonchai.datetime.picker.component.time.event.TimeSelectionModelEvent;
import dev.phonchai.datetime.picker.component.time.event.TimeSelectionModelListener;
import dev.phonchai.datetime.picker.event.TimeSelectionEvent;
import dev.phonchai.datetime.picker.event.TimeSelectionListener;
...
/**
 * A time picker component for Java Swing that supports both 12-hour and 24-hour
 * formats.
 * <p>
 * Features include:
 * <ul>
 * <li>12-hour (AM/PM) and 24-hour time selection</li>
 * <li>Thai and English localization</li>
 * <li>Clock-style visual selection interface</li>
 ...
 * picker.set24HourView(false);
 * picker.setLanguage(PickerLanguage.THAI);
 * picker.addTimeSelectionListener(e -> System.out.println(e.getTime()));
 * }</pre>
 *
 * @author dev.phonchai
 * @version 1.0.0
...
public class TimePicker extends PanelPopupEditor implements TimeSelectionModelListener {
    ...
    private DateTimeFormatter format12h;
    ...
    /**
     * Creates a new TimePicker with default settings.
     * <p>
     * Default configuration:
     * <ul>
     * <li>Language: Thai</li>
     * <li>Time format: 12-hour (AM/PM)</li>
    ...
    /**
     * Creates a new TimePicker with a custom time selection model.
     *
     * @param model the time selection model to use, or null to use default
     */
    ...
    private void updateFormatters() {
        format12h = DateTimeFormatter.ofPattern("hh:mm a", getDisplayLocale());
    }
    ...
    public void set24HourView(boolean hour24) {
        if (panelClock.isUse24hour() != hour24) {
            ...
            repaint();
            if (editor != null) {
                InputUtils.changeTimeFormatted(editor, hour24, getDisplayLocale(), header::isAm,
    ...
    public void setLanguage(PickerLanguage language) {
        ...
        if (editor != null) {
            InputUtils.changeTimeFormatted(editor, is24HourView(), getDisplayLocale(), header::isAm,
                    getInputValidationListener());
            setEditorValue();
        }
        repaint();
            ...
            valueCallback = value -> {
                if (value == null && isTimeSelected()) {
                    ...
                    if (value != null && !value.equals(getSelectedTimeAsString())) {
                        LocalTime time = InputUtils.stringToTime(panelClock.isUse24hour(), getDisplayLocale(),
                                value.toString());
                        if (time != null) {
    ...
    protected String getDefaultPlaceholder() {
        if (defaultPlaceholder == null) {
            if (is24HourView()) {
                ...
                defaultPlaceholder = "--:-- " + "-".repeat(getAmPmMaxLength(getDisplayLocale()));
            }
        }
        return defaultPlaceholder;
    }

    private static int getAmPmMaxLength(Locale locale) {
        try {
            String[] amPm = DateFormatSymbols.getInstance(locale != null ? locale : Locale.ENGLISH).getAmPmStrings();
