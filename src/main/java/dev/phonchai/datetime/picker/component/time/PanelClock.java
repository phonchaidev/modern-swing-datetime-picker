...
public class PanelClock extends JPanel {
    ...
    protected void paintSelection(Graphics2D g2, int x, int y, int size) {
        ...
        Area area = new Area(
                new Ellipse2D.Float(x + size - (centerSize / 2), y + size - (centerSize / 2), centerSize, centerSize));
        if ((hourSelectionView && timeSelectionModel.getHour() != -1)
                || (!hourSelectionView && timeSelectionModel.getMinute() != -1)) {
            area.add(new Area(new RoundRectangle2D.Float(x + size - (lineSize / 2), y + margin, lineSize, lineHeight,
                    lineSize, lineSize)));
            area.add(new Area(new Ellipse2D.Float(x + size - (selectSize / 2), y + margin - selectSize / 2, selectSize,
                    selectSize)));
            if (!hourSelectionView && !animationChange.isRunning() && (timeSelectionModel.getMinute() % 5 != 0)) {
                area.subtract(new Area(new Ellipse2D.Float(x + size - (unselectSize / 2), y + margin - unselectSize / 2,
    ...
    private void runAnimation(boolean animate) {
        int value = hourSelectionView ? timePicker.getTimeSelectionModel().getHour()
