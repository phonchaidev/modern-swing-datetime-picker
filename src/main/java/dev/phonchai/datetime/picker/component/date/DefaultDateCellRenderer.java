...
public class DefaultDateCellRenderer {
    ...
    public void paint(Graphics2D g2, DatePicker datePicker, ButtonDate component, SingleDate date, float width,
        ...
        if (dateSelectionModel.getDateSelectionMode() == DatePicker.DateSelectionMode.BETWEEN_DATE_SELECTED
     ...
     */
    protected void paintDateSelection(Graphics2D g2, DatePicker datePicker, ButtonDate component, boolean isSelected,
    ...
    protected void paintBetweenDateSelection(Graphics2D g2, DatePicker datePicker, ButtonDate component,
            SingleDate date, float width, float height, float arc) {
        ...
        if (!dateSelectionModel.getDate().same(getToDate(datePicker))) {
            ...
            if (date.same(getToDate(datePicker))) {
                if ((right && rowIndex != 0) || (!right && rowIndex != 6)) {
                    g2.fill(createShape(rec.x, rec.y, width, rec.height, arc, !right,
                            !component.isHover() && dateSelectionModel.getToDate() == null));
                }
            }
        }
    }

    protected void paintCurrentDate(Graphics2D g2, DatePicker datePicker, ButtonDate component, SingleDate date,
            boolean isSelected, float width, float height, float arc) {
        Rectangle2D.Float rec = getRectangle(width, height);
        ...
        if (isSelected) {
            ...
            area.subtract(new Area(FlatUIUtils.createComponentRectangle(rec.x + s, rec.y + s, rec.width - s * 2f,
                    rec.height - s * 2f, arc - s)));
        } else {
    ...
    private SingleDate getToDate(DatePicker datePicker) {
        DateSelectionModel dateSelectionModel = datePicker.getDateSelectionModel();
        return dateSelectionModel.getToDate() != null ? dateSelectionModel.getToDate()
